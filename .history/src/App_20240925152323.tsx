...
const App: React.FC = () => {
  ...
  const repeatProducts = [...Array(1)].fill(
    <ProductCard
  ...
  const sector_products = [...Array(2)].fill(repeatProducts);
  ...
  const collectImages = (urls: string[]) => {
    setImageUrls(prevUrls => new Set ([...prevUrls, ...urls]));
  };
    ...
    <div className="App">
      ...
      <div className="contents">
        {sectors.map((sector, index) => (
            <Sector key={index} products={sector.products} desc={sector.desc} collectImages={collectImages} />
        ))}
      </div>
      <DownloadBtn imageUrls={imageUrls} />
    </div>
  );
}
