...
const App: React.FC = () => {
  ...
  const [repeat_products, setProducts] = React.useState(repeat_product);
  const [descs, setTitles] = React.useState(desc);

  const products = [...Array(2)].fill(repeat_product);
  
  const sectors = [
    {
      repeat_products: repeat_products[0],
      desc: descs[0],
    },
    {
      sector_products: products[1],
      desc: descs[1],
    },
  ];
